...
| Python                 | v.3.9.7 and v.3.6 (for DeepLoc)| https://www.python.org/ |
...
|SRA toolkit | v.3.1.0;v.3.1.1 | https://biocontainers.pro/ |
...
|SAMtools | v.1.21 | https://biocontainers.pro/ |
|BEDTools | v.2.31.1 | https://biocontainers.pro/ |
|Minimap2 | v.2.28 | https://biocontainers.pro/ |
...
Runs differential expression analysis on _Eurytemora affinis_ RNA-Seq data, calling the helper script `rna_pipe_paired.R`. Also should execute `deseq2eurytemora.R` after downloading and processing read data for differential expression analysis.
...
#### v2_all_metazoan_monophyly_tests.ipynb
Performs constrained topology searches for metazoan monophyly for all HGT intervals in topological categories III and IV using `constrained_topo_search_all.sh` and then tree topology tests with `constrained_topo_tests_all.sh`.
#### non_chimeric_HGT_analysis.ipynb
